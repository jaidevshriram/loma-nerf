...
if __name__ == "__main__":
    ...
    # Create the MLP
    in_channels = 3 + (2 * 3 * num_encoding_functions) # for every point, we have sin/cos encoding, repeated for num_encoding_functions
    ws, bs = get_sample_mlp(in_channels=in_channels, out_channels=out_channels, num_layers=num_layers_mlp, filter_size=mlp_filter_size)
    ...
    for i in tqdm(range(num_iterations)):
        ...
        for chunk_idx in range(num_chunks):
            ...
            depth_values += np.random.randn(num_samples_along_ray) * (far - near) / num_samples_along_ray # Perturb the depth values

            sample_points = chunk_ray_origins[:, None, :] + chunk_ray_directions[:, None, :] * depth_values[None, :, None]

            # Incorporate positional encoding into the sample points
            sample_points_encoded = positional_encoding(sample_points, num_functions=num_encoding_functions)
